...
class TmuxClaudeRunner(ClaudeRunner):
        ...
        async with s._turn_lock:
            ...
            if kind == "permission":
                if pending_tool == "ExitPlanMode":
                    ...
                    if decided is None:
                        # Ambiguous answer to a binary allow/deny prompt: approve
                        # nothing, keep the prompt pending, and ask the agent to
                        # re-answer clearly. The PreToolUse hook is still parked
                        # (no decision file written), so the gate holds closed.
                        s.status = "needs_permission"
                        s._delta.append(
    ...
    def _write_decision(self, s: _TmuxSession, choice: str) -> bool | None:
        """Resolve a binary allow/deny answer and write the decision file the
        parked PreToolUse hook is waiting on. Returns True (allowed) or False
        (denied), or None when the answer was AMBIGUOUS — in which case NO
        decision is written, so the hook stays parked and nothing is approved
        and the caller can re-ask. Matching is the fail-closed, word-level,
        deny-first logic in decide_permission()."""
